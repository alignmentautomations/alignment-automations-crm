      ...
      <div className="detail-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="detail-body">
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {(() => {
                ...
                if (accounts.size === 0) return <span style={{ fontSize: 12, color: "#64748b" }}>No accounts required</span>;
                return Array.from(accounts).map(a => (
                  <span key={a} style={{ padding: "6px 12px", background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 6, fontSize: 11, fontWeight: 600, color: "#60a5fa" }}>{a}</span>
                ));
              })()}
            </div>
          </div>
          ...
          <div>
            ...
            {allTasks.length > 0 && (<><div className="task-progress-label">{pct}% complete</div><div className="task-progress"><div className="task-progress-bar" style={{ width: pct + "%" }} /></div></>)}
          </div>
          {["alignment", "clinic"].map(type => {
            ...
            return (
              <div key={type}>
                <div className="detail-section-title" style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8 }}>{label}</div>
                {sections.map(section => {
                  const sectionTasks = tasks.filter(t => t.section === section);
                  const sDone = sectionTasks.filter(t => t.done).length;
                  const isExp = expandedSections[prefix + "-" + section];
                  return (
                    <div key={section} style={{ marginBottom: 10 }}>
                      <div onClick={() => toggleSection(prefix + "-" + section)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: "rgba(11,17,33,0.6)", borderRadius: 6, cursor: "pointer", userSelect: "none" }}>
                        ...
                        <span style={{ fontSize: 10, color: "#64748b", fontFamily: "'JetBrains Mono', monospace" }}>{sDone}/{sectionTasks.length}</span>
                      </div>
                      {isExp && (
                ...
                <div className="add-task-row">
                  <input className="add-task-input" placeholder={`Add ${type === "alignment" ? "Alignment" : "Clinic"} task…`}
                    value={type === "alignment" ? newAlignmentTask : newClinicTask}
                    onChange={e => type === "alignment" ? setNewAlignmentTask(e.target.value) : setNewClinicTask(e.target.value)}
                    onKeyDown={e => {
                      const val = type === "alignment" ? newAlignmentTask : newClinicTask;
                      if (e.key === "Enter" && val.trim()) {
                        const updated = [...tasks, { id: uid(), name: val.trim(), done: false }];
                        onUpdate({ ...clinic, [type === "alignment" ? "alignmentTasks" : "clinicTasks"]: updated });
                        type === "alignment" ? setNewAlignmentTask("") : setNewClinicTask("");
                      }
                    }} />
...
const MOCK_ACTIVE = [
  { id: "a1", name: "Dr. Sarah Chen", clinic: "Pacific Spine & Wellness", trigger: "new_client", triggeredAt: Date.now() - 1000 * 60 * 60 * 2, status: "active", nextStep: "SMS in 46h", channel: "sms", sequence: "New Client Welcome" },
  { id: "a2", name: "Marcus Powell", clinic: "Revive Chiropractic", trigger: "missed_call", triggeredAt: Date.now() - 1000 * 60 * 8, status: "active", nextStep: "Email in 22h", channel: "email", sequence: "Missed Call Recovery" },
  { id: "a3", name: "Jenna Torres", clinic: "Coastal Healing Arts", trigger: "form_inquiry", triggeredAt: Date.now() - 1000 * 60 * 45, status: "completed", nextStep: "Done", channel: "email", sequence: "Form Inquiry Follow-up" },
];
    ...
    <>
      <div className="main-header">
        <div>
          ...
          <div className="page-subtitle">{sequences.filter(s => s.active).length} active sequences · {sequences.length} total</div>
        </div>
      ...
      <div className="main-content">
        ...
        <div style={{ display: "flex", gap: 2, marginBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.07)", paddingBottom: 0 }}>
          {[{ id: "dashboard", label: "Dashboard" }, { id: "sequences", label: "Sequences" }].map(t => (
            <button key={t.id} onClick={() => setFuTab(t.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "10px 16px", fontSize: 13, fontWeight: 600, color: fuTab === t.id ? "#f8fafc" : "#475569", borderBottom: fuTab === t.id ? "2px solid #2563eb" : "2px solid transparent", marginBottom: -1, transition: "all 0.15s" }}>{t.label}</button>
          ))}
        </div>
        ...
        {fuTab === "dashboard" && (
          <>
            <div className="stats-grid">
              {[
                { label: "Active Sequences", value: sequences.filter(s => s.active).length, color: "#3b82f6" },
                { label: "In Progress", value: MOCK_ACTIVE.filter(c => c.status === "active").length, color: "#4ade80" },
                { label: "Completed", value: MOCK_ACTIVE.filter(c => c.status === "completed").length, color: "#94a3b8" },
                { label: "Trigger Types", value: TRIGGER_TYPES.length, color: "#a78bfa" },
              ].map((s, i) => (
            ...
            <div className="clients-table-wrap">
              <div className="clients-table-header">
                ...
                <span style={{ color: "#475569", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>{MOCK_ACTIVE.length} contacts</span>
              </div>
              <table>
                <thead>
                  <tr><th>Contact</th><th>Trigger</th><th>Sequence</th><th>Next Step</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {MOCK_ACTIVE.map(c => {
                    const trig = getTrigger(c.trigger);
                    return (
                      <tr key={c.id}>
                        <td>
                          <div style={{ fontWeight: 600, color: "#f8fafc", fontSize: 13 }}>{c.name}</div>
                          <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>{c.clinic}</div>
                        </td>
                        <td>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, color: trig.color, fontSize: 12, fontWeight: 600 }}><span>{trig.icon}</span>{trig.label}</div>
                          <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>{timeAgo(c.triggeredAt)}</div>
                        </td>
                        <td style={{ color: "#94a3b8", fontSize: 13 }}>{c.sequence}</td>
                        <td>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#94a3b8", fontSize: 12 }}>
                            <span style={{ color: c.channel === "email" ? "#60a5fa" : "#a78bfa" }}>{c.channel === "email" ? <Icon.Mail /> : <Icon.Sms />}</span>{c.nextStep}
                          </span>
                        </td>
                        <td><FuBadge active={c.status === "active" ? true : c.status === "paused" ? false : null} /></td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {fuTab === "sequences" && (
          <>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
              <button className="btn-primary" onClick={() => setShowNewSeq(true)}><Icon.Plus /> New Sequence</button>
            </div>
                    ...
                    <div className="seq-card-header">
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          ...
                          <FuBadge active={seq.active} />
                        </div>
                        <div style={{ color: "#475569", fontSize: 12, marginTop: 3 }}>
                          {trig.label} · {seq.steps.length} steps · {seq.steps.filter(s => s.channel === "email").length} email · {seq.steps.filter(s => s.channel === "sms").length} SMS
                        </div>
                      </div>
                      {/* Step pills */}
                        ...
                        {seq.steps.map((s, i) => (
                          <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 3 }}>
                            ...
                            <div className="seq-step-pill" style={{ background: s.channel === "email" ? "rgba(96,165,250,0.15)" : "rgba(167,139,250,0.15)", color: s.channel === "email" ? "#60a5fa" : "#a78bfa" }}>
                              {s.channel === "email" ? <Icon.Mail /> : <Icon.Sms />}
                            </div>
                          </div>
                        ))}
                      </div>
                      <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                        <button className="btn-ghost" style={{ padding: "5px 10px", fontSize: 12 }} onClick={() => setPreviewSeq(expanded ? null : seq.id)}>{expanded ? "Hide" : "Preview"}</button>
                        <button className="btn-ghost" style={{ padding: "5px 8px" }} onClick={() => setSequences(prev => prev.map(s => s.id === seq.id ? { ...s, active: !s.active } : s))}>{seq.active ? <Icon.Pause /> : <Icon.Play />}</button>
                        <button className="btn-ghost" style={{ padding: "5px 8px" }} onClick={() => setEditingSeq(seq)}><Icon.Edit /></button>
                        <button className="btn-danger" onClick={() => setSequences(prev => prev.filter(s => s.id !== seq.id))}><Icon.Trash /></button>
                      </div>
                    </div>
                    {expanded && (
                        ...
                        {seq.steps.map((s, i) => (
                          <div key={s.id} className="seq-step-preview">
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: s.body ? 6 : 0 }}>
                              ...
                              <span style={{ color: s.channel === "email" ? "#60a5fa" : "#a78bfa" }}>{s.channel === "email" ? <Icon.Mail /> : <Icon.Sms />}</span>
                              <span style={{ color: "#94a3b8", fontSize: 12 }}>{i === 0 ? "Immediately" : `After ${s.delay} ${s.delayUnit}`}{s.channel === "email" && s.subject ? ` — "${s.subject}"` : ""}</span>
                            </div>
                            {s.body && <div style={{ color: "#64748b", fontSize: 12, lineHeight: 1.6, fontFamily: "monospace", whiteSpace: "pre-wrap", maxHeight: 72, overflow: "hidden" }}>{s.body.slice(0, 200)}{s.body.length > 200 ? "…" : ""}</div>}
                          </div>
                        ))}
                      </div>
                    )}
...
export default function App() {
  ...
  setClinics(prev => prev.map(c => {
    ...
    // Logic: If moved to "Onboarding Sent", start the "New Client Welcome"
    if (newStatus === "onboarding_sent") {
      updates.activeSequenceId = "seq_1"; // New Client Welcome ID
      updates.sequenceStepIndex = 0;
