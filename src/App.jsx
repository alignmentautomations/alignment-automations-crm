...
function TriggerModal({ sequences, clinics, onClose, onLaunched }) {
  ...
  // Pass the selected sequence ID back to the App component to be saved
  if (onLaunched) {
    onLaunched(selectedSeq || "manual_launch");
  }
...
export default function App() {
  ...
  const handleSequenceLaunch = useCallback(async (clinicId, sequenceId) => {
  const updates = {
  ...
  // 2. Persist to Cloudflare Database
  try {
    await db.update(clinicId, updates);
    showToast("Sequence active and saved to DB");
  } catch (err) {
      ...
      <div className="app">
        ...
        {triggerForClinic && (
          <TriggerModal 
            ...
            onLaunched={(sequenceId) => {
              handleSequenceLaunch(triggerForClinic.id, sequenceId);
            }}
          />
        )}
